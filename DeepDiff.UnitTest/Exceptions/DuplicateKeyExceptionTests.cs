...
namespace DeepDiff.UnitTest.Exceptions
{
    public class DuplicateKeyExceptionTests
    {
        ...
        public void SimpleKey_Hashtable()
        {
            ...
            var ex = Assert.Throws<DuplicateKeyException>(() => deepDiff.DiffMany(existingEntities, newEntities, cfg => cfg.SetHashtableThreshold(1)));
        ...
        public void SimpleKey_NoHashtable()
        {
            ...
            var ex = Assert.Throws<DuplicateKeyException>(() => deepDiff.DiffMany(existingEntities, newEntities, cfg => cfg.DisableHashTable()));
            Assert.Equal(typeof(EntityLevel2), ex.EntityType);
            Assert.Equal("DP_0", ex.Keys);
        }
