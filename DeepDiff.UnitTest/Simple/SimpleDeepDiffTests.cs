...
public partial class SimpleDeepDiffTests
{
    ...
    [Fact]
    public void TestMultipleChanges_MultipleEntities_Naive()
    {
        var existingEntities = Enumerable.Range(0, 10).Select(x => new EntityLevel0
        {
            Index = x,
        ...
        var deepDiff = diffConfiguration.CreateDeepDiff();
        var diff = deepDiff.DiffMany(existingEntities, newEntities, cfg => cfg.UsePrecompiledEqualityComparer(false).GenerateOperations(false));
        var results = diff.Entities.ToArray();
