...
internal class PropertyInfoExt
{
    ...
    {
        var getMethod = propertyInfo.GetGetMethod() ?? throw new MissingPropertyGetterException(type, propertyInfo.Name);
        var setMethod = propertyInfo.GetSetMethod() ?? throw new MissingPropertySetterException(type, propertyInfo.Name);

        // Getter dynamic method the signature would be :
        // object Get(object thisReference)
        // { return ((TestClass)thisReference).Prop; }
        var dmGet = new DynamicMethod("Get", typeof(object), [typeof(object)]);
        ...
        var dmSet = new DynamicMethod("Set", typeof(void), [typeof(object), typeof(object)]);
