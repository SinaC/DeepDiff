...
internal sealed class KeyConfiguration(Type entityType, IEnumerable<PropertyInfo> keyProperties)
{
    public IComparerByProperty EqualityComparer { get; private set; } = null!;

    public IReadOnlyCollection<PropertyInfoExt> KeyProperties { get; } = keyProperties.Select(x => new PropertyInfoExt(entityType, x)).ToArray();
