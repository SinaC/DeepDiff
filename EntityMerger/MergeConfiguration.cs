...
public class MergeConfiguration
{
    ...
    {
        if (!MergeEntityConfigurations.TryGetValue(typeof(TEntity), out var mergeEntityConfiguration))
    ...
    private void MergeUsingNavigationMany(PropertyInfo navigationProperty, PersistEntity existingEntity, PersistEntity calculatedEntity)
    {
        ...
        if (!MergeEntityConfigurations.TryGetValue(childType, out var childMergeEntityConfiguration))
        ...
        var convertedMergedChildren = EnumerableExtensions.ConvertToList(mergedChildren, childType);
        if (convertedMergedChildren.Count > 0)
            navigationProperty.SetValue(existingEntity, convertedMergedChildren);
    }
